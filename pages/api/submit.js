...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Look up enquiry_source list value ID dynamically.
    // lead_type is not an AdminValueList — pass "General" as a plain string.
    let phoneAgentSourceId = null;
    try {
      const enquirySourceList = await rexPost(token, "admin-value-lists/getListValues", {
        list_name: "enquiry_source",
      });
      const rows = enquirySourceList.result || [];
      const match = rows.find((v) => v.text === "Phone Agent");
      if (match) phoneAgentSourceId = match.id;
    } catch (e) {
      console.error("Could not fetch enquiry_source list values:", e.message);
    }
    ...
    await rexPost(token, "leads/create", {
      data: {
        ...
        lead_type: "General",
        lead_source: phoneAgentSourceId ? { id: phoneAgentSourceId } : undefined,
      },
    });
